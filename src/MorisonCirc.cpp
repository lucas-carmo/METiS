...
vec::fixed<6> MorisonCirc::hydroForce_relWaveElev_inertia(const ENVIR &envir, const vec::fixed<3> &refPt) const
{
	vec::fixed<6> force(fill::zeros);
...
vec::fixed<6> MorisonCirc::hydroForce_relWaveElev_drag(const ENVIR& envir, const vec::fixed<3>& refPt) const
{
	vec::fixed<6> force(fill::zeros);
	...
	vec::fixed<3> u1_wl(this->u1(envir, m_numNodesBelowWL - 1));
	u1_wl -= arma::dot(u1_wl, m_zvec_sd) * m_zvec_sd;
